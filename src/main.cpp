...
// Copyright (c) 2013-2014 The Aphrodite developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aphrodite: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aphrodite Signed Message:\n";
...
{
    // Aphrodite: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 129600); // Aphrodite

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Aphrodite: 1 days
static const int64 nTargetSpacing = 1 * 60; // Aphrodite: 1 Minute Blocks
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aphrodite: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aphrodite: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Aphrodite: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "What Would You Do For A Aphrodite Block?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Aphrodite: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AphroditeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AphroditeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AphroditeMiner(CWallet *pwallet)
{
    printf("AphroditeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Aphrodite-miner");
    ...
    try { loop {
        ...
        printf("Running AphroditeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AphroditeMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AphroditeMiner, pwallet));
}
